...
const API_URL =
  ...
  window.location.origin.includes("127.0.0.1")
    ? "http://127.0.0.1:5000"
    : "https://linear-regression-backend.onrender.com";
let currentData = [];
...
const metricsPanelContainer = document.getElementById(
    ...
    if (stepNumber === 2) {
      ...
      if (workspaceSubtitle)
        workspaceSubtitle.innerHTML =
          "Before an AI model can be trained, you must physically structure the data into a Scatter Plot.";
      ...
      if (workspaceTitle)
        workspaceTitle.innerHTML = "Step 3: Train Your Model ⚙️";
      if (workspaceSubtitle)
        workspaceSubtitle.innerHTML =
          "The AI algorithm uses <strong>Linear Regression</strong> to draw a trendline. Your goal is to manually adjust the business assumptions below to perfectly fit the historical data.";
      if (step2Controls) step2Controls.classList.add("hidden");
      if (step3Controls) step3Controls.classList.remove("hidden");
      if (metricsPanelContainer)
        metricsPanelContainer.classList.remove("hidden");
...
function updateHints(currentSlope, currentIntercept) {
  ...
  if (
    Math.abs(intDiffNorm) > Math.abs(slopeDiffNorm) &&
    Math.abs(intDiffNorm) > 0.05
  ) {
    hint =
      intDiffNorm > 0
        ? "The model line is too low. Try <strong>increasing</strong> the Fixed Baseline (Intercept)."
        : "The model line is too high. Try <strong>decreasing</strong> the Fixed Baseline (Intercept).";
  } else if (Math.abs(slopeDiffNorm) > 0.05) {
    hint =
      slopeDiffNorm > 0
        ? "The angle is wrong. Try <strong>increasing</strong> the Strategic Impact (Slope)."
        : "The angle is wrong. Try <strong>decreasing</strong> the Strategic Impact (Slope).";
  }
...
const errorLinesPlugin = {
  ...
  afterDraw: (chart) => {
    ...
    const allXValues = missingPoints
      .filter((p) => p !== null)
      .map((p) => p.x)
      .concat(xValues);
    const dataMinX =
      allXValues.length > 0 ? Math.min(...allXValues) - 5 : x.min;
    const dataMaxX =
      allXValues.length > 0 ? Math.max(...allXValues) + 5 : x.max;
...
function initChart() {
  ...
  chartInstance = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: {
          labels: { color: "#1e293b", font: { family: "Outfit", size: 14 } },
        },
...
function runForecast() {
  ...
  forecastOutputY.innerHTML = predY.toLocaleString(undefined, {
...
if (autoFitBtn) {
  autoFitBtn.addEventListener("click", () => {
    ...
    const finalSlope = Math.max(
      parseFloat(slopeSlider.min),
      Math.min(parseFloat(slopeSlider.max), slope),
    );
    const finalInt = Math.max(
      parseFloat(interceptSlider.min),
      Math.min(parseFloat(interceptSlider.max), intercept),
    );
    ...
    const autofitInterval = setInterval(() => {
      ...
      const currentSlope =
        startSlope + (finalSlope - startSlope) * easeProgress;
